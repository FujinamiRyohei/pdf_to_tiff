...
class PDFtoTIFFConverter:
    ...
    def execution(self, event, context):
        try:
            ...
            content_type = self._utils.get_content_type_from_header(input_data["headers"])
            input_filename = self._utils.get_input_filename_from_header(input_data["headers"])
            output_file_extention = self._utils.get_output_file_extention_from_header(input_data["headers"])
            ...
            # 一時ファイルにPDFを保存
            input_file_path = self._utils.save_input_file(input_filename, input_file)
